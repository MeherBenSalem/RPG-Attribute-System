		...
		if (!JaumlConfigLib.getBooleanValue(directory, filename, "lock")) {
			for (String iterator : JaumlConfigLib.getArrayAsList(directory, filename, "cmd_to_exc")) {
				...
				if (stringCommand.contains("[param(")) {
					...
					if (commandParam > 0) {
						for (int index0 = 0; index0 < (int) (entity.getCapability(MemoryOfThePastModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new MemoryOfThePastModVariables.PlayerVariables())).attribute_3; index0++) {
							finalValue = finalValue + commandParam;
						}
					}
				} else {
					finalValue = (entity.getCapability(MemoryOfThePastModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new MemoryOfThePastModVariables.PlayerVariables())).attribute_3;
				}
