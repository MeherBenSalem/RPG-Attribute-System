		...
		if (!JaumlConfigLib.getBooleanValue(directory, filename, "lock")) {
			...
			for (int index0 = 0; index0 < (int) JaumlConfigLib.getArrayLength(directory, filename, "cmd_to_exc"); index0++) {
				...
				if (stringCommand.contains("[param(")) {
					...
					if (commandParam > 0) {
						for (int index1 = 0; index1 < (int) (entity.getCapability(MemoryOfThePastModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new MemoryOfThePastModVariables.PlayerVariables())).attribute_3; index1++) {
