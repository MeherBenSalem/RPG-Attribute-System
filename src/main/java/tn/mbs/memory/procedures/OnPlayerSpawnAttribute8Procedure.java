...
public class OnPlayerSpawnAttribute8Procedure {
	public static void execute(Entity entity) {
		...
		if (!JaumlConfigLib.getBooleanValue(directory, filename, "lock")) {
			for (String iterator : JaumlConfigLib.getArrayAsList("text", "text", "cmd_to_exc")) {
				...
				if (stringCommand.contains("[param(")) {
					...
					if (commandParam > 0) {
						for (int index0 = 0; index0 < (int) (entity.getCapability(MemoryOfThePastModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new MemoryOfThePastModVariables.PlayerVariables())).attribute_8; index0++) {
							finalValue = finalValue + commandParam;
						}
					}
				} else {
					finalValue = (entity.getCapability(MemoryOfThePastModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new MemoryOfThePastModVariables.PlayerVariables())).attribute_8;
				}
