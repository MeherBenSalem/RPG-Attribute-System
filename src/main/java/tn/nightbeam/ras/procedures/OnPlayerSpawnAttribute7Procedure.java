...
public class OnPlayerSpawnAttribute7Procedure {
	public static void execute(Entity entity) {
		...
		if ((entity instanceof LivingEntity _livingEntity0 && _livingEntity0.getAttributes().hasAttribute(RpgAttributeSystemModAttributes.ATTRIBUTE_7.get())
				? _livingEntity0.getAttribute(RpgAttributeSystemModAttributes.ATTRIBUTE_7.get()).getBaseValue()
				: 0) == 0) {
			for (String iterator : JaumlConfigLib.getArrayAsList(directory, filename, "cmd_to_exc")) {
				...
				if (stringCommand.contains("[param(")) {
					...
					if (commandParam > 0) {
						for (int index0 = 0; index0 < (int) (entity.getCapability(RpgAttributeSystemModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new RpgAttributeSystemModVariables.PlayerVariables())).attribute_7; index0++) {
							finalValue = finalValue + commandParam;
						}
					}
				} else {
					finalValue = (entity.getCapability(RpgAttributeSystemModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new RpgAttributeSystemModVariables.PlayerVariables())).attribute_7;
				}
