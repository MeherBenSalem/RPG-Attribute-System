		...
		if ((entity instanceof LivingEntity _livingEntity0 && _livingEntity0.getAttributes().hasAttribute(RpgAttributeSystemModAttributes.ATTRIBUTE_8.get())
			...
			for (String iterator : JaumlConfigLib.getArrayAsList(directory, filename, "cmd_to_exc")) {
				...
				if (stringCommand.contains("[param(")) {
					...
					if (commandParam > 0) {
						for (int index0 = 0; index0 < (int) (entity.getCapability(RpgAttributeSystemModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new RpgAttributeSystemModVariables.PlayerVariables())).attribute_8; index0++) {
							finalValue = finalValue + commandParam;
						}
					}
				} else {
					finalValue = (entity.getCapability(RpgAttributeSystemModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new RpgAttributeSystemModVariables.PlayerVariables())).attribute_8;
				}
