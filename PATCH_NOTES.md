...
- **HUD respects Hide GUI (F1)** — the RAS level/VP overlay now hides with the vanilla HUD when Hide GUI is on.

### Upgrade

Replace the loader jar for your Minecraft version with **4.2.6**. Server and clients should run the same RAS version.
...
- **Admin command OP gate** — `/ras add attributes`, `xp`, `set xp`, `reset`, `unlock`, and `lock` now require OP level 4 (same as `/ras add level`).
...
Operator restrictions, NeoForge vs Forge jar selection, HUD/GUI text shadow (`gui_shadow_color`), and attribute 11+ lock/icon behaviour. See `docs/listing-notes.md` for CurseForge/Modrinth reply text.
