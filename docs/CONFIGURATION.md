...
See `docs/HEALTH_SCALING.md` for worked examples.
...
**What it does:** The maximum **total attribute value** (not points invested). Once the calculated value reaches this cap, the player cannot allocate more points. With `init_val_attribute: 20` and `max_level: 40`, a player can invest at most 20 points when `base_value_per_point` is 1.0.
...
"max_level": 500
```
Players can reach a total attribute value of up to `max_level` (including the base `init_val_attribute`).
