...
public class PlayerStatsGUIScreen extends AbstractContainerScreen<PlayerStatsGUIMenu>
    ...
    public void extractContents(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        guiGraphics.blit(RenderPipelines.GUI_TEXTURED,
            Identifier.tryParse("rpg_attribute_system:textures/screens/bg.png"), this.leftPos + -61,
        ...
        for (int i = 0; i < visible.size(); i++) {
            ...
            if (isUnlocked || isLocked) {
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED,
                    Identifier.tryParse("rpg_attribute_system:textures/screens/iconbg.png"),
                        this.leftPos + getIconBgX(col), iconBgY, 0, 0, 24, 24, 24, 24);
            }

            // Attribute icon (use dynamic icon from config with namespace support)
            if (isUnlocked || isLocked) {
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, AttributeManager.getAttributeIconLocation(attrId),
            ...
            if (isLocked) {
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED,
                        Identifier.tryParse("rpg_attribute_system:textures/screens/bg_attributes_locked.png"),
                        this.leftPos + getAttrBgX(col), bgY, 0, 0, 97, 30, 97, 30);
            }
        }
    ...
    protected void extractLabels(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
        ...
        guiGraphics.text(this.font,
                ...
                false);
        guiGraphics.text(this.font, ReturnCurrentLevelProcedure.execute(entity), 68, 1, -1, false);
        guiGraphics.text(this.font,
                Component.translatable("gui.rpg_attribute_system.player_stats_gui.label_available_points"), 125, 2,
                -16777216, false);
        guiGraphics.text(this.font, ReturnExtraPointsProcedure.execute(entity), 207, 2, -1, false);
        ...
        for (int i = 0; i < visible.size(); i++) {
            ...
            if (DisplayLogicAttributeGenericProcedure.execute(entity, attrId)) {
                ...
                guiGraphics.text(this.font, value, getLabelX(col), labelY, -1, false);
            }
            // Locked attributes show no number (hidden)
        }

        // Modifier and XP
        guiGraphics.text(this.font, ReturnCurrentModifierProcedure.execute(entity), 259, 14, -1, false);
        guiGraphics.text(this.font, CurrentXpToLevelProcedure.execute(entity), -43, 1, -16777216, false);

        // Page indicator
        if (getTotalPages() > 1) {
            String pageText = (currentPage + 1) + "/" + getTotalPages();
            guiGraphics.text(this.font, pageText, 100, 175, -1, false);
        }
    }
