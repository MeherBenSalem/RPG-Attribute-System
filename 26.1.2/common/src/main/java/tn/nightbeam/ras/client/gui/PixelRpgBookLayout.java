...
public final class PixelRpgBookLayout {
    ...
    public void update(int screenWidth, int screenHeight) {
        int availableWidth = Math.max(1, Math.min(screenWidth - MIN_MARGIN * 2,
                Math.round(screenWidth * VIEWPORT_WIDTH_RATIO)));
        int availableHeight = Math.max(1, Math.min(screenHeight - MIN_MARGIN * 2,
                Math.round(screenHeight * VIEWPORT_HEIGHT_RATIO)));
        float widthScale = availableWidth / (float) DESIGN_WIDTH;
        float heightScale = availableHeight / (float) DESIGN_HEIGHT;
