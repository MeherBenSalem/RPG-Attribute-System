...
public class PlayerStatsOverviewScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        int header = 0xFF000000 | (StatsDisplayConfig.getHeaderColor() & 0xFFFFFF);
        graphics.text(font, title.getString(), width / 2 - font.width(title) / 2, 12, header, false);
        ...
        for (String line : lines) {
            if (y >= top - LINE_HEIGHT && y <= bottom) {
                graphics.text(font, stripColorCodesForWidth(line), 20, y, 0xFFFFFFFF, false);
            }
            y += LINE_HEIGHT;
        }
